...
-- ============================================
-- Fixes:
--   1. Block anon writes to scoring result tables (ranking_proveedores, provider_criterion_scores)
--   2. Create RPC for secure weight-based score recalculation
--   3. Server-side token expiration enforcement via RLS
--   4. Audit log for all scoring changes
--   5. Lock scoring config after evaluation starts
-- ============================================

BEGIN;

-- Use desarrollo schema (change to 'public' for production deployment)
...
CREATE TABLE IF NOT EXISTS desarrollo.scoring_audit_log (
...
CREATE INDEX IF NOT EXISTS idx_scoring_audit_log_project ON desarrollo.scoring_audit_log(project_id);
CREATE INDEX IF NOT EXISTS idx_scoring_audit_log_created ON desarrollo.scoring_audit_log(created_at DESC);

-- RLS: read-only for anon (only SECURITY DEFINER functions can write)
ALTER TABLE desarrollo.scoring_audit_log ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow read audit log" ON desarrollo.scoring_audit_log;
CREATE POLICY "Allow read audit log" ON desarrollo.scoring_audit_log
...
CREATE OR REPLACE FUNCTION log_scoring_changes()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'UPDATE' THEN
        INSERT INTO desarrollo.scoring_audit_log (project_id, table_name, operation, provider_name, old_data, new_data)
        VALUES (
            NEW.project_id,
            TG_TABLE_NAME,
            TG_OP,
    ...
    ELSIF TG_OP = 'INSERT' THEN
        INSERT INTO desarrollo.scoring_audit_log (project_id, table_name, operation, provider_name, new_data)
        VALUES (
            NEW.project_id,
            TG_TABLE_NAME,
            TG_OP,
    ...
    ELSIF TG_OP = 'DELETE' THEN
        INSERT INTO desarrollo.scoring_audit_log (project_id, table_name, operation, provider_name, old_data)
        VALUES (
            OLD.project_id,
            TG_TABLE_NAME,
            TG_OP,
...
DROP TRIGGER IF EXISTS trigger_audit_ranking_proveedores ON desarrollo.ranking_proveedores;
CREATE TRIGGER trigger_audit_ranking_proveedores
    AFTER INSERT OR UPDATE OR DELETE ON desarrollo.ranking_proveedores
    FOR EACH ROW
    EXECUTE FUNCTION log_scoring_changes();

-- Apply audit trigger to scoring_weight_configs
DROP TRIGGER IF EXISTS trigger_audit_scoring_weight_configs ON desarrollo.scoring_weight_configs;
CREATE TRIGGER trigger_audit_scoring_weight_configs
    AFTER INSERT OR UPDATE OR DELETE ON desarrollo.scoring_weight_configs
    FOR EACH ROW
    EXECUTE FUNCTION log_scoring_changes();

-- Apply audit trigger to scoring_categories
DROP TRIGGER IF EXISTS trigger_audit_scoring_categories ON desarrollo.scoring_categories;
...
DROP POLICY IF EXISTS "Allow read ranking_proveedores" ON desarrollo.ranking_proveedores;
CREATE POLICY "Allow read ranking_proveedores" ON desarrollo.ranking_proveedores
    FOR SELECT USING (true);
...
DROP POLICY IF EXISTS "Allow read provider_criterion_scores" ON desarrollo.provider_criterion_scores;
CREATE POLICY "Allow read provider_criterion_scores" ON desarrollo.provider_criterion_scores
    FOR SELECT USING (true);
...
CREATE OR REPLACE FUNCTION recalculate_scores_with_weights(
    p_project_id UUID,
    p_weights JSONB
)
RETURNS JSONB AS $$
...
BEGIN
    ...
    FOR v_provider IN
        ...
        FROM desarrollo.ranking_proveedores
    ...
    LOOP
        ...
        LOOP
            ...
            FOR v_criterion IN
                ...
                FROM desarrollo.scoring_criteria scr
            ...
            LOOP
                ...
                IF v_score > 10 THEN v_score := v_score / 10; END IF;
                -- actual_weight = criterion_weight_within_category * category_weight / 100
            ...
            -- Category score = weighted sum / total weight
        ...
        UPDATE desarrollo.ranking_proveedores
        SET overall_score = ROUND(v_new_overall, 2),
            category_scores_json = v_category_scores,
            -- Update legacy columns if present in category scores
...
-- Replace the overly permissive policies on supplier_upload_tokens
...
DROP POLICY IF EXISTS "Allow read supplier_upload_tokens" ON desarrollo.supplier_upload_tokens;
CREATE POLICY "Allow read supplier_upload_tokens" ON desarrollo.supplier_upload_tokens
    FOR SELECT USING (true);

-- INSERT: allow (for creating new tokens from the app)
DROP POLICY IF EXISTS "Allow insert supplier_upload_tokens" ON desarrollo.supplier_upload_tokens;
CREATE POLICY "Allow insert supplier_upload_tokens" ON desarrollo.supplier_upload_tokens
    FOR INSERT WITH CHECK (true);

-- UPDATE: only allow if token is NOT expired (server-side enforcement)
DROP POLICY IF EXISTS "Allow update non_expired_tokens" ON desarrollo.supplier_upload_tokens;
CREATE POLICY "Allow update non_expired_tokens" ON desarrollo.supplier_upload_tokens
...
CREATE POLICY "Allow update non_expired_qa_tokens" ON desarrollo.qa_response_tokens
    FOR UPDATE USING (expires_at > NOW());


-- NOTE: Scoring config locking (categories/criteria) deferred to next version
-- for legal compliance in public sector procurement.
-- For now, weights remain editable at all times as a feature.
